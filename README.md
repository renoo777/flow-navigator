...
**自动更新**：Windows NSIS 安装版启动时会自动检查 GitHub Releases 新版本（v0.1.4+），发现新版弹窗确认后即下载安装、自动重启，无需手动重装。也可在主页「❤ 支持作者」面板里点「检查更新」手动触发。
...
主页（流程图库）「变量导航器」标题旁点「❤ 支持作者」即可看到微信/支付宝收款码（双码已嵌入，离线可用）；亦可通过下方通道：

- **微信赞赏码**（renoo）/ **支付宝收款码**（周先生的小铺 \*明） — 见应用内「支持作者」面板，含收款人备注，付款前请核对
